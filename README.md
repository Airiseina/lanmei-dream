...
| random_beauty | 随机美图：`/随机美图`；从可配置的 Random Mage 兼容接口取 Pixiv 插画，经固定分级、元数据和视觉模型审核后发送 |

命令举例：`/签到`、`/帮助`（`/help` 也行）、`/答题 go python 困难`、`/每日一句`、`/随机美图`、`/删除表情 <标签>`、`/插件 安装 <url>`（远程装 Wasm 插件）。自然语言也能触发命令（意图分析，连 at 段都能保留）。

`random_beauty` 默认使用第三方公共实例 `https://i.mukyu.ru`，不保证上游可用性。插件固定排除 R18、AI 作品和保守的擦边标签，并对下载后的实际图片字节做视觉审核；视觉模型缺失、审核超时或结论不确定时拒绝发送。单次最多尝试 2 张，总超时 18 秒，同一时刻只执行一个取图任务，避免占满 Conduit Worker。回复会附带作者和 Pixiv 原作链接，图片版权归原作者。
