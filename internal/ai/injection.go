...
// 定位为 LLM 防护指令（chat.go 安全规则）之外的独立闸门：LLM 声明"忽略操纵内容"是软防线，
// 本地正则检测是硬观测，命中时记录日志供行为观测与后续策略（拦截/降权）参考。
// 模式用 [^，。！？\n]{0,6} 限制动词与宾语间隔，避免跨句误报，也降低正常聊天的误报。
...
// 返回命中的模式描述（中文）；未命中返回空串。供调用方记录日志/决定策略；英文模式大小写不敏感。
