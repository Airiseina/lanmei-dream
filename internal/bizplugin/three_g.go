...
// ThreeGPlugin 实现关键词触发科普：普通消息（含私聊）文本含 "3G"/"3g" 时，
// 回复 [@对方 + 重邮 3G 科普文本] 一条消息，不做防刷限流；
// 直接读 MessageContext 的 RawMsg，不依赖 bot/gateway 包。
//
// 插件 ID three_g；无命令、无工具；不依赖对象存储、LLM、视觉服务、StateStore 或受限 KV。
...
// Execute 触发 3G 科普：记录日志后经出站段键 bot.send.segments 组装
// [@对方 + 固定科普文本] 一条消息发送（at 段永远按 OneBot 12 语义用 user_id，
// 协议差异由 hub.SendSegments 收敛）。
// 由 plugin.three_g.pipeline.main 在 containsThreeG 命中普通消息中的 "3G"/"3g" 后调用，
// 不做防刷限流；UserID 为空（异常情况）时降级为纯文本固定文案。
func (pass *threeGPass) Execute(ctx *conduit.MessageContext) error {
	...
	// 出站段：[@对方 + 科普文本]，永远按 OneBot 12 语义组装（at 段用 user_id）；
