...
// 提及分类：决定群消息"是否应回复"的强/弱/无三档信号（LLM 主判官方案）。
// at（平台 ID 精确命中）为免费且精确的强信号，不依赖 LLM；其余提及（呼格/主语/祈使宾语/
// 关系从句/条件句/话题标记/情感对象/指代等）由意图分析 LLM 返回的 LinguisticJudge 判定，
// 按置信度划分强/弱；不再使用硬编码的文本结构规则（无法适配自然语言，易漏判）。
...
// MentionResult 提及分类结果（Manager.classifyMention 的输出）。
type MentionResult struct {
	// Mentioned 是否提及了机器人（含强/弱两档）。
	Mentioned bool
	// Mode 提及模式（MentionAt / MentionLinguistic；未提及时为 MentionNone）。
	Mode   MentionMode
